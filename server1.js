/**
 * ========================================================================================================
 * File: sicbo_prediction_api_god_tier_ultimate_ai_v6.js (Fixed with normalizeHistory)
...
// ================================================================
// === Các thuật toán phân tích & dự đoán nâng cao ===
// ================================================================
// (Markov chain, balance, heuristic, predictAdvanced, predictSums)
// === Giữ nguyên như code của bạn, không thay đổi ===
// ================================================================
...
const analyzeMultiTimeframeBalance = (statusHistory) => {
  ...
  if (shortTermDiff >= 4) {
    ...
    return { prediction, certainty, description: `Cân bằng: Cầu ngắn hạn lệch. Dự đoán cân bằng lại.` };
  }
  if (longTermDiff >= 7) {
    const prediction = longTermCounts.T > longTermCounts.X ? 'X' : 'T';
    const certainty = Math.min(longTermDiff * 8, 75);
    return { prediction, certainty, description: `Cân bằng: Cầu dài hạn lệch. Có khả năng đảo chiều.` };
  }
  return { prediction: null, certainty: 0, description: "Cân bằng: Cầu ổn định." };
};
...
const analyzeHeuristicPatterns = (statusHistory) => {
  ...
  if (recent[0] !== recent[1] && recent[1] === recent[2] && recent[2] === recent[3]) {
    ...
    return { prediction, certainty: 75, description: "Heuristic: Nhận diện mô hình 3-2-1. Dự đoán theo đảo." };
  }
  if (recent[0] !== recent[1] && recent[1] !== recent[2] && recent[2] === recent[3]) {
    const prediction = recent[1];
    return { prediction, certainty: 70, description: "Heuristic: Nhận diện mô hình 1-1-2. Dự đoán cầu ngắn." };
  }
  return { prediction: null, certainty: 0, description: "Heuristic: Không tìm thấy mẫu hình." };
};

const dynamicWeights = { markov: 1.0, balance: 1.0, heuristic: 1.0 };

const predictAdvanced = (history) => {
  console.log('--------------------------------------------------');
  console.log('[AI Prediction] Bắt đầu quá trình dự đoán...');
  if (history.length < 15) {
    console.log('[AI Prediction] Không đủ dữ liệu để dự đoán nâng cao.');
    return { du_doan: "Chưa đủ dữ liệu", doan_vi: [], confidence: 0, giai_thich: "Cần ít nhất 15 phiên để phân tích." };
  }
  ...
  for (const model of allModels) {
    if (model.result.prediction) {
      ...
      predictionBreakdown[model.name] = { prediction: model.result.prediction, certainty: model.result.certainty, score };
    }
  }
  ...
  if (totalScore > 0) {
    ...
    finalPrediction = 'Tài';
    finalConfidence = 50;
    explanation.push("Các mô hình không có cơ sở, dự đoán ngẫu nhiên.");
  }
  ...
  console.log('[AI Prediction] Kết quả cuối cùng:', { du_doan: finalPrediction, doan_vi: predictedSums, confidence: finalConfidence, giai_thich: explanation.join(' ') });
  console.log('--------------------------------------------------');
  return { du_doan: finalPrediction, doan_vi: predictedSums, confidence: finalConfidence, giai_thich: explanation.join(' ') };
};
...
const predictSums = (history, taixiu_prediction) => {
  ...
  return Object.entries(sumScores).sort(([, a], [, b]) => b - a).slice(0, 3).map(([sum]) => parseInt(sum));
};
...
app.use((err, req, res, next) => {
  ...
  res.status(err.status || 500).json({ error: { message: err.message || 'Đã xảy ra lỗi máy chủ.' } });
});
...
app.get('/api/sicbo/lxk', async (req, res, next) => {
  try {
    ...
    console.log(`[API Request] Lấy thành công dữ liệu lịch sử. Số phiên: ${history.length}`);
    if (!Array.isArray(history) || history.length < 15) {
      return res.status(200).json({
        ...
        doan_vi: [],
        do_tin_cay: "0%",
        giai_thich: "Cần ít nhất 15 phiên để phân tích.",
        luu_y: "yeu a ko mấy tk em"
      });
    }
    ...
    const result = {
      ...
      luu_y: "Cảnh báo: Đây là công cụ phân tích thống kê, không chắc chắn 100%.",
      lien_he: "@cskhtoollxk"
    };
    res.json(result);
  } catch (err) {
...
app.listen(PORT, () => {
  console.log(`✅ API Sicbo Prediction đang chạy tại http://localhost:${PORT}`);
  console.log(`⚠️ Lưu ý: Đây là công cụ phân tích thống kê, không đảm bảo thắng tuyệt đối.`);
});